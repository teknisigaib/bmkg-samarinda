...
export default function AwsDashboardUI({ initialData }: Props) {
  ...
  return (
    <div className="min-h-screen text-slate-800 pb-20 bg-slate-50/50">
       <div className="w-full mx-auto pt-6 lg:px-8 max-w-[1600px] space-y-6">
        ...
        <section className="relative flex flex-col items-center justify-center text-center mb-10 max-w-3xl mx-auto pt-2">
           ...
           <p className="relative z-10 text-sm md:text-base text-slate-500 leading-relaxed font-medium px-4 max-w-4xl mb-8">
              Data Pengamatan alat pengamatan cuaca otomatis (Automatic Weather Station). Menampilkan kondisi aktual dari stasiun {activeStation.name}.
           </p>
           ...
           <div className="relative z-10 flex flex-wrap justify-center items-center bg-white border border-slate-200 rounded-2xl shadow-sm p-1">
              ...
              <div className="flex items-center gap-2 px-4 py-1.5 border-r border-slate-100">
                 ...
                 <span className={`text-xs font-semibold uppercase tracking-widest ${!isStale ? 'text-emerald-700' : 'text-amber-700'}`}>
                    {!isStale ? 'Online' : 'Gangguan'}
                 </span>
              </div>
              
              {/* Selector */}
              <div className="flex items-center gap-2 px-2 py-0.5 border-r border-slate-100 relative group">
                 <MapPin className="absolute left-3 w-3.5 h-3.5 text-blue-500 z-10 pointer-events-none" />
                 <select 
                    value={selectedStationId}
                    onChange={(e) => handleStationChange(e.target.value)}
                    className="appearance-none bg-transparent pl-9 pr-8 py-1 rounded text-xs font-bold text-slate-700 focus:outline-none cursor-pointer hover:text-blue-600 transition-colors z-20"
                 >
              ...
              <div className="flex items-center gap-2 px-4 py-1.5">
                 <Clock className={`w-4 h-4 ${isLoading ? 'text-blue-500 animate-spin' : 'text-slate-400'}`} />
                 <span className="text-xs font-medium text-slate-500">
                    Sync: {data.lastUpdateTime} UTC
                 </span>
              </div>
           </div>
        </section>
        ...
        {isStale && (
          <div className="mb-6 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-center text-amber-800 text-sm font-semibold flex items-center justify-center gap-2 animate-in slide-in-from-top-2">
             <div className="bg-white rounded-full p-1"><AlertTriangle className="w-4 h-4 text-amber-600" /></div>
