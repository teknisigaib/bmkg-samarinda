...
export default function RunningText({ weatherText }: RunningTextProps) {
  
  const isSafe = weatherText.includes("Tidak ada peringatan") || 
                weatherText.toLowerCase().includes("nihil") ||
                weatherText.includes("berakhir");
  ...
  const styles = isSafe 
    ...
    : {
        ...
        speed: "60s" // Sedikit lebih cepat agar peringatan segera terbaca
      };
  ...
  return (
    <div className={`${styles.bg} ${styles.border} ${styles.textColor} border-b text-sm font-semibold h-10 overflow-hidden relative flex items-center z-40 shadow-sm transition-colors duration-500`}>
      ...
      <div className="flex-1 overflow-hidden relative h-full flex items-center">
        <div 
            className="animate-marquee whitespace-nowrap absolute flex items-center gap-12"
            style={{ animationDuration: styles.speed }}
        >
          {/* Loop teks dua kali untuk efek continuous yang mulus */}
