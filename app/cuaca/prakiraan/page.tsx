...
export default function Home() {
  ...
  // TAMBAHAN: Efek untuk mengubah Judul Tab Browser
  useEffect(() => {
    if (weather) {
      ...
      <div className="w-full mx-auto px-4 sm:px-6 pt-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
           <div className="order-2 md:order-1 flex-1 overflow-hidden">
             ...
             {/* --- PERUBAHAN 2: Breadcrumb Dinamis --- */}
             {/* Sekarang teksnya otomatis mengambil dari data parent (Provinsi) */}
             <div className="text-sm text-slate-400 flex items-center gap-1">
                {weather.parentLocation && (
                  <>
                    <span className="hover:text-slate-600 cursor-pointer" onClick={() => {
                        // Opsional: Logika untuk kembali ke parent ID bisa ditambahkan di sini
                        // misal: setCurrentLocationId(weather.parentId)
                    }}>
                      {weather.parentLocation}
                    </span>
                    <span>&gt;</span>
        ...
        <header>
          <div className="flex items-center gap-1.5 text-blue-600 font-bold text-xs mb-2 uppercase tracking-wider">
            ...
            {/* Translate Level ke Bahasa Indonesia agar lebih rapi */}
        ...
        {/* Logic Grid Sub-Region:
            Jika level = Desa (village), jangan tampilkan grid (karena tidak punya anak).
            Jika level = Provinsi/Kota/Kecamatan, tampilkan grid anak-anaknya.
        */}
